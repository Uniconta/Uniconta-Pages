...
namespace UnicontaISO20022CreditTransfer
{
    public class RmtInf
    {
        ...
        private readonly CompanyBankENUM companyBank;
        ...
        private const string RFRDDOCAMT = "RfrdDocAmt";
        private const string DUEPYBLAMT = "DuePyblAmt";
        ...
        public RmtInf(List<string> remittanceInfoUnstructList, string remittanceInfo, string ocrPaymentId, string ocrPaymentType, CreditTransferDocument doc)
        {
            ...
            if (doc != null)
            {
                ...
                this.paymentAmount = doc.PaymentAmount;
                this.companyBank = doc.CompanyBank;
            }
        }
        ...
        internal virtual void Append(BaseDocument baseDoc, XmlDocument doc, XmlElement parent)
        {
            ...
            if (unstructuredList != null || (ocrPaymentType != null && !string.IsNullOrEmpty(ocrPaymentId)))
            {
                ...
                if (unstructuredList != null && ocrPaymentType != BaseDocument.QRR)
                ...
                if (ocrPaymentType!= null)
                {
                    ...
                    if (companyBank == CompanyBankENUM.SEB && currencyCode != null && paymentAmount != 0)
                    {
                        XmlElement rfrdDocAmt = baseDoc.AppendElement(doc, strd, RFRDDOCAMT);
                        baseDoc.AppendElement(doc, rfrdDocAmt, DUEPYBLAMT, paymentAmount).SetAttribute(BaseDocument.CURRENCY_ID, currencyCode);
                    }
                    ...
                    if (ocrPaymentType == BaseDocument.OCR || ocrPaymentType == BaseDocument.NETS)
                    {
                        baseDoc.AppendElement(doc, cdOrPrtry, CD, SCOR);
                        baseDoc.AppendElement(doc, cdtrRefInf, REF, ocrPaymentId);
                    }
                    else if (ocrPaymentType == BaseDocument.QRR)
                    {
                        baseDoc.AppendElement(doc, cdOrPrtry, PRTRY, BaseDocument.QRR);
                        baseDoc.AppendElement(doc, cdtrRefInf, REF, ocrPaymentId);
                    }
                }
            }
